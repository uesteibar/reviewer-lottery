...
class ConfigTester {
	...
	private checkRuleForDuplicates(
		...
		if (selectors.includes("*") && selectors.length > 1) {
			...
			const coloredSelectors = otherSelectors.map((s) => this.colorizeGroupName(s)).join(", ");
			...
			if (selector.startsWith("!")) {
				...
				if (explicitGroups.length > 0) {
					...
					if (overlappingGroups.length > 0) {
						const coloredOverlapping = overlappingGroups.map((g) => this.colorize(g, "group")).join(", ");
						const coloredRuleName = this.colorizeRuleName(ruleName);
		...
		if (exclusionPatterns.length > 1) {
			const coloredPatterns = exclusionPatterns.map((p) => this.colorize(`"${p}"`, "group")).join(", ");
			const coloredRuleName = this.colorizeRuleName(ruleName);
			const warningIcon = this.colorize("⚠", "warning");
			warnings.push(
